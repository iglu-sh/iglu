...
export default class FilesystemProvider extends StorageProvider {
    ...
    public override init():void{
        ...
        if(!FilesystemProvider.basepath){
            if(!process.env.FILESYSTEM_DIRECTORY){
                throw new Error('panic(files::FilesystemProvider) No valid Filesystem directory given in process environment')
            }
            FilesystemProvider.basepath = process.env.FILESYSTEM_DIRECTORY
        } 
        if(!fs.existsSync(FilesystemProvider.basepath)){
            Logger.debug(`Filesystem basepath does not exist, creating it at ${FilesystemProvider.basepath}`)
    ...
    * */
    public override async delete(tenant: string, name: string): Promise<void> {
        ...
        if(!fs.existsSync(requested_path)){
            ...
            throw new Error(`warn(files::FilesystemProvider): Tried to unlink a file that does not exist`)
        }
    ...
    * */
    public override async createTenant(tenant:string):Promise<void>{
        const requested_path = `${FilesystemProvider.basepath}/${tenant}`
        if(fs.existsSync(requested_path)){
            Logger.debug(`Tenant directory: ${requested_path} already exists and it was tried to create that again`)
            throw new Error(`warn(files::FilesystemProvider): Tried to create a directory which already exists`)
        }
    ...
    public override async store(tenant: string, name: string, data: Buffer): Promise<void> {
        ...
        if(!fs.existsSync(`${FilesystemProvider.basepath}/${tenant}`)){
            ...
            if(!fs.existsSync(`${FilesystemProvider.basepath}/${tenant}`)){
                ...
                throw new Error(`panic(files::FilesystemProvider): Could not create tenant directory`)
            }
        }
    ...
    * */
    public override async combine(tenant:string, upload_id:string, hash:string, name:string, parts: Array<part>):Promise<void>{
        // Fetch all files in the folder
        const all_files = await this.getAll(tenant)
        if(all_files === null){
            throw new Error('panic(files::FilesystemProvider): Could not read all files from directory and thus cannot combine requested files for given upload id')
        }
        
        const finalFilePath = `${FilesystemProvider.basepath}/${tenant}/${name}` 

        for(const part_item of parts){
            if(!fs.existsSync(`${FilesystemProvider.basepath}/${tenant}/${upload_id}.part-${part_item.partNumber}`)){
                Logger.error(
            ...
            fs.appendFileSync(finalFilePath, fs.readFileSync(`${FilesystemProvider.basepath}/${tenant}/${upload_id}.part-${part_item.partNumber}`))
            fs.unlinkSync(`${FilesystemProvider.basepath}/${tenant}/${upload_id}.part-${part_item.partNumber}`)
        }
