...
export default class Logger {
    ...
    static log(level: LogLevel, message: string) {
        ...
        if (Logger.jsonLogging) {
            ...
            if (Logger.logStream) {
                Logger.logStream(
                    `${Logger.prefixText} ${new Date().toISOString()} [${level}] ${message}\n`,
                );
            }
        }
    }
    ...
    static logRequest(endpoint: string, method: string) {
        if (Logger.jsonLogging) {
            ...
            console.log(
                Logger.customChalk.grey(`${new Date().toISOString()}`),
                `${Logger.verbColorMap[method](method)} `,
                endpoint,
            );
        }
    }
    static logResponse(endpoint: string, method: string, statusCode: number) {
        if (Logger.jsonLogging) {
            ...
            const statusCodeColor = Logger.statusCodeColorMap[`${statusCodeGroup}xx`]
                ? Logger.statusCodeColorMap[`${statusCodeGroup}xx`]
                : Logger.statusCodeColorMap.default;
            console.log(
                Logger.customChalk.grey(`${Logger.prefix} ${new Date().toISOString()}`),
