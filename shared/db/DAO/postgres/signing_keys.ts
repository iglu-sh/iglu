...
import { api_keys, api_keys_tenants_link, signing_keys } from "../../schema_pg";
import type { signing_keys_abstract } from "../abstracts/signing_keys_abstract";
export default class postgres_signing_keys implements signing_keys_abstract {
    ...
    public async insert(item: signing_key): Promise<signing_key> {
        ...
        const return_values = await this.db.transaction(async (tx) => {
            ...
            if (!return_value || return_value.length > 1 || !return_value[0]) {
                Logger.error(
                    "Panic(DB::DAO::signing_keys::postgres_signing_keys): Could not insert into signing_keys table! (Unknown Error)",
                );
                throw new Error(
                    "Panic(DB::DAO::signing_keys::postgres_signing_keys): Could not insert into signing_keys table?",
                );
            }
        ...
        if (!return_values?.[0] || return_values.length !== 1) {
            Logger.error(
                "Panic(DB::DAO::signing_keys::postgres_signing_keys): Could not insert into signing_keys table! (Did not receive a record from transaction)",
            );
            throw new Error(
                "Panic(DB::DAO::signing_keys::postgres_signing_keys): Could not insert into signing_keys table! (Did not receive a record from transaction)",
            );
    ...
    public async getById(id: string): Promise<signing_key | null> {
        ...
        if (db_data.length > 1) {
            Logger.error(
                "Panic(DB::DAO::signing_keys::postgres_signing_keys): Got more than one returned in getByID, I do not know what to do with this",
            );
            throw new Error(
                "Panic(DB::DAO::signing_keys::postgres_signing_keys): Got more than one returned in getByID, I do not know what to do with this",
            );
    ...
    public async update(to_update: signing_key): Promise<signing_key> {
        return await this.db.transaction(async (tx) => {
            ...
            if (updated_item.length === 0 || !updated_item[0]) {
                Logger.error(
                    "Panic(DB::DAO::signing_keys::postgres_signing_keys): Did not get an updated item back from the signing_keys table",
                );
                throw new Error(
                    "Panic(DB::DAO::signing_keys::postgres_signing_keys): Did not get an updated item back from the signing_keys table",
                );
            ...
            if (updated_item.length > 1) {
                Logger.error(
                    "Panic(DB::DAO::signing_keys::postgres_signing_keys): Updated more than one record, although it was compared by ID",
                );
                throw new Error(
                    "Panic(DB::DAO::signing_keys::postgres_signing_keys): Updated more than one record, although it was compared by ID",
                );
