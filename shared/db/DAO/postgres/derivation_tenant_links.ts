...
import {
    api_keys,
    derivations,
    derivations_tenants_links,
    signing_keys,
    tenants,
} from "../../schema_pg";
import type { derivation_tenant_links_abstract } from "../abstracts/derivation_tenant_links_abstract";

export default class postgres_derivation_tenant_link implements derivation_tenant_links_abstract {
    ...
    public async insert(item: derivation_tenant_link): Promise<derivation_tenant_link> {
        ...
        const results = await this.db.transaction(async (tx) => {
            const new_record = await tx
                .insert(derivations_tenants_links)
                .values(record_to_insert)
                .returning();

            if (!new_record?.[0]) {
                Logger.error(
                    "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not insert into derivation_tenant_link table (Did not get returning value from Statement)",
                );
                throw new Error(
                    "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not insert into derivation_tenant_link table (Did not get returning value from Statement)",
                );
            ...
            const records = await tx
                .select({
                    ...
                    csig: derivations_tenants_links.csig,
                })
                .from(derivations_tenants_links)
                .innerJoin(
                    derivations,
                    eq(derivations_tenants_links.derivations_id, derivations.id),
                )
                .innerJoin(tenants, eq(derivations_tenants_links.tenants_id, tenants.id))
                .innerJoin(
                    signing_keys,
                    eq(derivations_tenants_links.signing_keys_id, signing_keys.id),
                )
        ...
        if (!results?.[0] || results.length !== 1) {
            Logger.error(
                "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not insert into derivation_tenant_link table (Did not get returning value from Transaction or multiple returned)",
            );
            throw new Error(
                "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not insert into derivation_tenant_link table (Did not get returning value from Transaction or multiple returned)",
            );
    ...
    public async getAll(): Promise<Array<derivation_tenant_link>> {
        const records = await this.db
            ...
            .innerJoin(tenants, eq(derivations_tenants_links.tenants_id, tenants.id))
            .innerJoin(signing_keys, eq(derivations_tenants_links.signing_keys_id, signing_keys.id))
            .innerJoin(api_keys, eq(api_keys.id, signing_keys.api_keys_id));
    ...
    public async getById(id: string): Promise<derivation_tenant_link | null> {
        ...
        if (records.length > 1) {
            Logger.error(
                "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not get by ID: more than one record returned for an ID",
            );
            throw new Error(
                "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not get by ID: more than one record returned for an ID",
            );
    ...
    public async update(to_update: derivation_tenant_link): Promise<derivation_tenant_link> {
        const results = await this.db.transaction(async (tx) => {
            const updated_record = await tx
                .update(derivations_tenants_links)
                .set({
            ...
            if (!updated_record?.[0]) {
                Logger.error(
                    "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not update record: Nothing returned from Database",
                );
                throw new Error(
                    "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not update record: Nothing returned from Database",
                );
            ...
            if (updated_record.length > 1) {
                Logger.error(
                    "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not update record: More than one record updated",
                );
                throw new Error(
                    "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not update record: More than one record updated",
                );
        ...
        if (!results?.[0]) {
            Logger.error(
                "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not update record: Nothing returned from Transaction",
            );
            throw new Error(
                "Panic(DB::DAO::derivation_tenant_link::postgres_derivation_tenant_link): Could not update record: Nothing returned from Transaction",
            );
