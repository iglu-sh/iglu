...
import { api_keys } from "../../schema_pg";
import type { api_key_abstract } from "../abstracts/api_key_abstract";

export default class postgres_api_keys implements api_key_abstract {
    ...
    public async insert(item: api_key): Promise<api_key> {
        ...
        if (!inserted_record?.[0] || inserted_record.length !== 1) {
            Logger.error(
                "Panic(DB::DAO::requests::postgres_api_keys): Could not insert into api_keys table! (Either nothing returned or more than one record returned)",
            );
            throw new Error(
                "Panic(DB::DAO::requests::postgres_api_keys): Could not insert into api_keys table! (Either nothing returned or more than one record returned)",
            );
    ...
    public async getById(id: string): Promise<api_key | null> {
        ...
        if (record.length !== 1) {
            Logger.error(
                "Panic(DB::DAO::requests::postgres_api_keys): Could not fetch from api_keys table! (More than one record for an ID)",
            );
            throw new Error(
                "Panic(DB::DAO::requests::postgres_api_keys): Could not fetch from api_keys table! (More than one record for an ID)",
            );
    ...
    public async getByHash(to_find: string): Promise<api_key | null> {
        ...
        if (record.length > 1) {
            Logger.error(
                "Panic(DB::DAO::requests::postgres_api_keys): Could not fetch from api_keys table! (More than one record for a hash)",
            );
            throw new Error(
                "Panic(DB::DAO::requests::postgres_api_keys): Could not fetch from api_keys table! (More than one record for a hash)",
            );
    ...
    public async update(to_update: api_key): Promise<api_key> {
        ...
        if (!updated_record?.[0] || updated_record.length === 0) {
            Logger.error(
                "Panic(DB::DAO::requests::postgres_api_keys): Could not update api_keys table! (More than one record returned after update)",
            );
            throw new Error(
                "Panic(DB::DAO::requests::postgres_api_keys): Could not fetch from api_keys table! (More than one record returned after update)",
            );
        }
