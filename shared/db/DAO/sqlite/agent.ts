...
export class agent_sqlite implements agent_abstract{
    private db = new SQLiteConnector().getDB()
     
    public async insert(item:agent):Promise<agent>{
        return await this.db.transaction(async(tx)=>{
            const inserted_records = await tx.insert(agents).values({
                ...item,
                id: undefined,
            ...
            if(!inserted_records[0]){
                Logger.error(
                    'panic(DAO::sqlite::agent): Could not insert into agents table: Did not get returning value from insert statement'
                )
                throw new Error(
                    'panic(DAO::sqlite::agent): Could not insert into agents table: Did not get returning value from insert statement'
                )
            ...
            .then((result)=>{
                if(!result[0]){
                    Logger.error(
                        'panic(DAO::sqlite::agent): Could not insert into agents table: Did not get value from select statement'
                    )
                    throw new Error(
                        'panic(DAO::sqlite::agent): Could not insert into agents table: Did not get value from select statement'
                    )
    ...
    public async update(item:agent):Promise<agent>{
        return await this.db.transaction(async(tx)=>{
            ...
            if(!updated[0]){
                    Logger.error(
                        'panic(DAO::sqlite::agent): Could not update agents table: Did not get returning value from update statement'
                    )
                    throw new Error(
                        'panic(DAO::sqlite::agent): Could not update agents table: Did not get returning value from update statement'
                    )
            ...
            .then((result)=>{
                if(!result[0]){
                    Logger.error(
                        'panic(DAO::sqlite::agent): Could not update agents table: Did not get returning value from select statement'
                    )
                    throw new Error(
                        'panic(DAO::sqlite::agent): Could not update agents table: Did not get returning value from select statement'
                    )
