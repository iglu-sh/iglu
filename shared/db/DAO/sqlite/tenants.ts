...
export default class sqlite_tenants{
    ...
    public async insert(item:tenant):Promise<tenant>{
        ...
        if(inserted_items.length === 0 || !inserted_items[0]){
            Logger.error('Panic(DB::DAO::tenants::sqlite_tenants): Could not insert into tenants table! (Unknown Error)')
            throw new Error('Panic(DB::DAO::tenants::sqlite_tenants): Could not insert into tenants table?')
        }
    ...
    * */
    public async getById(id:string):Promise<tenant | null>{
        ...
        if(db_data.length > 1){
            Logger.error("Panic(DB::DAO::tenants::sqlite_tenants): Got more than one returned in getByID, I do not know what to do with this")
            throw new Error("Panic(DB::DAO::tenants::sqlite_tenants): Got more than one returned in getByID, I do not know what to do with this")
        } 
   ...
   public async update(to_update:tenant){
        const updated_item = await this.db.update(tenants).set({
        ...
        if(updated_item.length === 0 || !updated_item[0]){
            Logger.error("Panic(DB::DAO::tenants::sqlite_tenants): Did not get an updated item back from the tenants table")
            throw new Error("Panic(DB::DAO::tenants::sqlite_tenants): Did not get an updated item back from the tenants table")
        }
        if(updated_item.length > 1){
            Logger.error("Panic(DB::DAO::tenants::sqlite_tenants): Updated more than one record, although it was compared by ID")
            throw new Error("Panic(DB::DAO::tenants::sqlite_tenants): Updated more than one record, although it was compared by ID")
        }
