...
import { agents, agents_deployments_links, deployment_keys, deployments, tenants } from "../../schema_sqlite";
import type { agent_deployment_link_abstract } from "../abstracts/agent_deployment_link_abstract";
import type { agents_deployments_link,agent } from "@/db_types";
import { eq } from "drizzle-orm";
import { alias } from "drizzle-orm/sqlite-core";

const agent_keys = alias(deployment_keys, 'agent_keys')

export class sqlite_agents_deployments_links implements agent_deployment_link_abstract{
    ...
    public async insert(item: agents_deployments_link): Promise<agents_deployments_link> {
        return await this.db.transaction(async(tx)=>{
            const created = await tx.insert(agents_deployments_links)
            .values({
                ...item,
                id: undefined,
            ...
            if(!created[0]){
                Logger.error(
                    'panic(DAO::sqlite::agents_deployments_links): Could not insert into agents_deployments_links table: Did not get returning value from insert method'
                )
                throw new Error(
                    'panic(DAO::sqlite::agents_deployments_links): Could not insert into agents_deployments_links table: Did not get returning value from insert method'
                )
    ...
    public async delete(item:agents_deployments_link):Promise<void>{
        await this.db.delete(agents_deployments_links).where(eq(agents_deployments_links.id, item.id))
    }

    public async update(item:agents_deployments_link):Promise<agents_deployments_link>{
        await this.db.transaction(async(tx)=>{
            const updated = await tx.update(agents_deployments_links)
            .set({
                ...item,
                id: undefined,
                deployments_id: item.deployments_id.id,
            ...
            if(!updated[0]){
                Logger.error(
                    'panic(DAO::sqlite::agents_deployments_links): Could not update agents_deployments_links table: Did not get returning value from update method'
                )
                throw new Error(
                    'panic(DAO::sqlite::agents_deployments_links): Could not update agents_deployments_links table: Did not get returning value from update method'
                )
