...
export class sqlite_deployment_keys implements deployment_key_abstract {
    ...
    public async insert(item:deployment_key):Promise<deployment_key>{
        ...
        return await this.db.transaction(async(tx)=>{
            ...
            const inserted_record = await tx.insert(deployment_keys)
            .values(val_to_insert)
            .returning()
            
            if(!inserted_record?.[0]){
                Logger.error('panic(DAO::sqlite::deployment_keys): Could not insert into deployment_keys table: Did not get inserted record back from insert statement')
                throw new Error('panic(DAO::sqlite::deployment_keys): Could not insert into deployment_keys table: Did not get inserted record back from insert statement')
            }
            ...
            return await tx.select({
                ...
                .then((result)=>{
                    if(!result?.[0]){
                        Logger.error('panic(DAO::sqlite::deployment_keys): Could not insert into deployment_keys table: Did not get inserted record back from select statement')
                        throw new Error('panic(DAO::sqlite::deployment_keys): Could not insert into deployment_keys table: Did not get inserted record back from select statement')
                    }
                    return result[0]
                })
        })
    ...
    public async update(item:deployment_key):Promise<deployment_key>{
        return await this.db.transaction(async(tx)=>{
            const updated_record = await tx.update(deployment_keys)
            .set({
                ...item,
                id: undefined,
                tenants_id: item.tenants_id.id
            ...
            if(!updated_record[0]){
                Logger.error('panic(DAO::sqlite::deployment_keys): Could not update  deployment_keys table: Did not get returning value for update statement')
                throw new Error('panic(DAO::sqlite::deployment_keys): Could not update deployment_keys table: Did not get returning value for update statement')
            }
