...
export default class Tenants extends DAO<tenant_item>{
    ...
    public override async insert(item: tenant_item): Promise<tenant_item> {
        ...
        if(!return_item){
            Logger.error('Panic(DB::DAO::tenants): Tenant insert failed (did not receive return value)')
            throw new Error('Panic(DB::DAO::tenants): Tenant insert failed (did not receive return value)')
        }
        return return_item 
    }
    ...
    public override async update(item: tenant_item): Promise<tenant_item> {
        ...
        if(!return_item){
            Logger.error('Panic(DB::DAO::tenants): Did not get an updated item in the update function.')
            throw new Error('Panic(DB::DAO::tenants): Did not get an updated item in the update function.')
        }
