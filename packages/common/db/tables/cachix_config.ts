import {Table} from "./Table.ts";
import type {cache_builder_key as table_type} from "@iglu-sh/types/core/db";
export class cachix_config extends Table {
    private data: table_type[] = [];
    public async getData():Promise<table_type[]> {
        return this.data
    }
    public async getById(id:string):Promise<table_type> {
        const entry = this.data.find((bl) => bl.id === id)
        if(!entry){
            throw new Error(`Entry with id ${id} not found`)
        }
        return entry
    }
    public async init(): Promise<void> {
        this.data = await this.query(`
            SELECT
                cc.id,
                cc.push,
                cc.buildoutputdir,
                row_to_json(c.*) as target,
                (
                    SELECT
                        json_build_object(
                                'id', b.id,
                                'cache', row_to_json(bc.*),
                                'name', b.name,
                                'description', b.description,
                                'enabled', b.enabled,
                                'trigger', b.trigger,
                                'cron', b.cron,
                                'created_at', b.created_at,
                                'updated_at', b.updated_at,
                                'arch', b.arch,
                                'webhookurl', b.webhookurl)
                    FROM cache.builder b
                             INNER JOIN cache.cache bc ON b.cache = bc.id
                    WHERE b.id = cc.builder
                ) as builder,
                (
                    SELECT
                        json_build_object(
                                'cache', row_to_json(cbk_c.*),
                                'id', cbk.id,
                                'plaintext_apikey', cbk.plaintext_apikey,
                                'plaintext_signingkey', cbk.plaintext_signingkey,
                                'signingkey', (
                                    SELECT
                                        json_build_object(
                                                'id', psk.id,
                                                'api_key',json_build_object(
                                                        'id', ak_psk.id,
                                                        'user', row_to_json(u_psk),
                                                        'name', ak_psk.name,
                                                        'hash', ak_psk.hash,
                                                        'description', ak_psk.description,
                                                        'created_at', ak_psk.created_at,
                                                        'last_used', ak_psk.last_used
                                                          ),
                                                'name', psk.name,
                                                'public_key', psk.public_signing_key,
                                                'description', psk.description,
                                                'created_at', psk.created_at
                                        )
                                    FROM cache.public_signing_key psk
                                             INNER JOIN cache.api_key ak_psk ON psk.api_key = ak_psk.id
                                             INNER JOIN cache.user u_psk ON ak_psk."user" = u_psk.id
                                    WHERE psk.id = cbk.signingkey
                                ),
                                'api_key', json_build_object(
                                        'id', ak.id,
                                        'user', row_to_json(u.*),
                                        'name', ak.name,
                                        'hash', ak.hash,
                                        'description', ak.description,
                                        'created_at', ak.created_at,
                                        'last_used', ak.last_used
                                           )
                        )
                    FROM cache.cache_builder_key cbk
                             INNER JOIN cache.cache cbk_c ON cbk.cache = cbk_c.id
                             INNER JOIN cache.api_key ak ON ak.id = cbk.apikey
                             INNER JOIN cache.user u ON u.id = ak."user"
                    WHERE cbk.id = cc.cache_builder_key
                ) as cache_builder_key
            FROM cache.cachix_config cc
                     INNER JOIN cache.cache c ON cc.target = c.id
        `).then((res)=>{
            return res.rows as table_type[]
        })
    }
}
