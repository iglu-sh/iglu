...
import type { cache } from "@iglu-sh/types";
...
export default function CacheDropdown({ caches }: { caches: cache[] }) {
    ...
    useEffect(() => {
        if (
            !caches.find((cache) => cache.id.toString() === cacheID && window)
        ) {
    ...
    return (
        <div className="flex flex-row items-center gap-2">
            <DropdownMenu>
                <DropdownMenuTrigger className="flex h-full w-full flex-row items-start p-2">
                    <div className="flex w-full flex-col items-start gap-1">
                        <strong>
                            {caches.find(
                                (cache) => cache.id.toString() === cacheID,
                            )?.name ?? "Select Cache"}
                        </strong>
                ...
                <DropdownMenuContent align="start" className="w-[205px]">
                    {caches.map((cache, index) => {
                        return (
                            ...
                            >
                                <Link
                                    href={`${pathname}?cacheID=${cache.id}`}
                                    key={cache.id}
                                    className="w-full"
                                >
                    ...
                    <DropdownMenuItem>
                        <Link
                            href={`/app/caches/create?cacheID=${cacheID}`}
