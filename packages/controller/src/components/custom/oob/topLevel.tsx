...
export default function OOBTopLevel(){
    ...
    useEffect(()=>{
         console.log(session)
        if(!session?.data?.user?.session_user?.is_admin && session.status != "loading" ){
    ...
    return(
        <div className="m-7 flex flex-col gap-4">
                ...
                </div>
                {
                    session?.data ? (
                        <div className="flex flex-row gap-2 justify-center items-center">
                            <Avatar style={{backgroundColor: session.data.user.session_user.avatar_color}}
                                    className="flex justify-center items-center"
                            >{session.data.user.session_user.username[0]}</Avatar>
                            <span className="ml-1">{session.data.user.session_user.username}</span>
                        </div>
                    ) : null
                }
            </div>
            ...
            <Card>
                <CardContent className="flex flex-row justify-between items-center">
                    ...
                    <div className="flex flex-row gap-1">
                        ...
                        >{
                            claimed.length > 0 ? "Unclaim All Caches" : "Claim All Caches"
                        }</Button>
            ...
            <div className="flex flex-row justify-end">
                <AddDialogue caches={caches} claimedCaches={claimed} userID={session.data?.user?.id ?? ""}/>
            </div>
        </div>
    )
}