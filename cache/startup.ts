    ...
    * */
    ...
    if(config.tenants.create_tenants_from_config){
        ...
        for(const requested_tenant of config.tenants.definitions){
            ...
            const already_exists = all_tenants.filter((item)=>item.name === requested_tenant.name)[0]
            if(already_exists){
                ...
                if(api_key_id !== 'generated'){
                    ...
                    const link_exists = await new Api_keys_tenants_link().getByTenantAndKey(api_key_id, tenant.id)
            ...
            // If the cache does not exist, we create it
                ...
                if(api_key_id === 'generated'){
                    ...
                    Logger.info(`Created API key for cache: ${requested_tenant.name}; Key: ${key.raw}`)
                    Logger.warn('Keep in mind that you will need to upload a signing key using cachix before you can push to your cache though!')
                }
                ...
                const all_tenants_for_key = await new Api_keys_tenants_link().getByApiKey(api_key_id) 

                // If this is true then the api key is not linked to the tenant
                if(all_tenants_for_key.filter((link_entry)=>{return link_entry.tenants_id.id === created_tenant.id}).length === 0){
                    ...
                    Logger.debug(`Key with ID: ${api_key_id} is already linked to cache with id ${created_tenant.id}`)
                }
            }
        }
    }
