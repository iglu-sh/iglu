...
export const post = [
    ...
    async(req:Request, res: Response) => {
        ...
        const is_only_strings = BODY_ARRAY.filter((x)=>{
        ...
        if(tenant.length !== 1 || !tenant[0]){
            Logger.debug('Narinfo request returned inconclusive database response (no tenant found or multiple with same name, cannot continue)')
            return res.status(404).json(MakeRestResponse(
                404,
                'Not found',
                true,
