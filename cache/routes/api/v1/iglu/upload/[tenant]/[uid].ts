...
export const put = [
    async(req:Request, res:Response)=>{
        ...
        try{
            ...
            Logger.debug(`Wasn't able to verify params or query in upload request`)
        ...
        writeable_request_stream.on("finish", async ()=>{
            ...
            if(buffer_md5_hash !== upload.md5){
                ...
                return res.status(400).json(MakeRestResponse(
                    ...
                    true,
                    {
                        'error_description': 'The upload got corrupted, try again (Md5 Hash mismatch)'
                    }
                ))
            }

            try{
                await new Filesystem().store(upload.tenants_id.id, `${upload.id}.part-${verified_query.partNumber}`, buffer_to_store)
            }
            catch(e){
