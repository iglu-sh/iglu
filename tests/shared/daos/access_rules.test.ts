...
* */
async function test_access_rules_table(access_rules_dao:access_rules_abstract, db_type:SupportedDatabasesString | 'Facade'){
    ...
    ******* */
    test.serial(`${db_type} (DAO, ${table_name}): Expect tenant insert to create at least one access_rule`, async()=>{
        ...
        const access_rules_in_db_for_created_tenant = await access_rules_dao.getByTenant(tenant_to_use.id)
    ...
    test.serial(`${db_type} (DAO, ${table_name}: Expect standalone access_rule insert to work and adhere to zod schem: Expect standalone access_rule insert to work and adhere to zod schema`, async ()=>{
    ...
    ******* */
    test.serial(`${db_type} (DAO, ${table_name}: Expect to get back at least two record when calling getAll()`, async ()=>{
    ...
    test.serial(`${db_type} (DAO, ${table_name}: Expect to get a record back when querying using its id`, async ()=>{
    ...
    test.serial(`${db_type} (DAO, ${table_name}: Expect to get the catchall record for the test tenant when querying any IP`, async()=>{
        expect(tenant_db).toBeDefined()
    ...
    test.serial(`${db_type} (DAO, ${table_name}: Expect a deleted rule to actually disappear`, async ()=>{
        expect(access_rule_for_update).toBeDefined()
        access_rule_for_update = access_rule_for_update as access_rule
        await access_rules_dao.delete(access_rule_for_update)

        const rule_that_was_just_deleted = await access_rules_dao.getById(access_rule_for_update.id)
    ...
    ******* */
    test.serial(`${db_type} (DAO, ${table_name}: Expect the auto-created rule to delete itself after the associated tenant is delted)`, async ()=>{
        expect(tenant_db).toBeDefined()
        expect(access_rule_db).toBeDefined
        tenant_db = tenant_db as tenant
        access_rule_db = access_rule_db as access_rule
    ...
    test.serial(`${db_type} (DAO, ${table_name}: Expect a rule insert associated with a non-existent tenant to fail`, async ()=>{
        expect(tenant_db).toBeDefined()
        tenant_db = tenant_db as tenant
        ...
        try{
            await access_rules_dao.insert({
                id: 'n/a',
                tenants_id: tenant_db,
                ip_block: '0.0.0.0/0',             start_ip: ip_range.range_start,
                end_ip: ip_range.range_end,
                priority: 1,
                action: 'drop',
                name: 'Test rule' 
            ...
            Logger.debug(`Got expected error from access_rules_dao insert to non-existing tenant: ${e}`)
