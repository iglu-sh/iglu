...
async function createTestFile(upload_id:string, split:boolean):Promise<{hash:string, size_in_bytes:number, split_into:number}>{
    ...
    if(split === true){
        ...
        fs.copyFileSync(`/tmp/iglu-test/file.bin`, `/tmp/iglu/${tenant_to_use.id}/${upload_id}.part-1`)
    }
