...
function writeTestFile(derivation_to_use: derivation){
    fs.writeFileSync(`/tmp/iglu/${tenant_to_use.id}/${derivation_to_use.cstorehash}-${derivation_to_use.cstorehash}.${derivation_to_use.compression}`, 'Just something cool')
}
...
test("Expect a GET request that tries to access a derivation that does not have a file associated with it to fail", async () =>{
    ...
    fs.rmSync(`/tmp/iglu/${tenant_to_use.id}/${derivation.cstorehash}-${derivation.cstorehash}.${derivation.compression}`)
